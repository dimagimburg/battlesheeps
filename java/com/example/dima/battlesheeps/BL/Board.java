...
public class Board implements Serializable {
        ...
        for (int num=0;num<mShipNumbers.length;num++) {
            for(int j=0;j<mShipNumbers[num];j++) {
                ...
                while (!finished) {
                    ...
                    direction = r.nextBoolean(); //true=down false=right
    ...
    public String Play(int coordX,int coordY){
        if (board[coordX][coordY].getStatus()!= null)//if not null already played
            return "fired";
        else{
            if (board[coordX][coordY].getmShip()!=null){ //not empty
